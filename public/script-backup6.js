/* Automated fixes by DeepSeek and some manual fixes done with the help of Ron_B */
...
class PokerGameSystem {
  ...
  setupTable() {
    ...
    if(window.isBanter) {
      Array.from(this.parent.querySelectorAll("[look-at]")).forEach(ele => {
  ...
  correctTempBet() {
    ...
    if(this.currentPlayer && this.tempBet > this.currentPlayer.chips) {
  ...
  setTempBet(bet) {
    if(this.currentPlayerSection && this.currentPlayer) {
      const newTempBet = this.tempBet + bet;
      if(newTempBet >= 0 && newTempBet <= this.currentPlayer.chips) {
        this.tempBet = newTempBet;
        this.setTempBetText();
      }
    }
  }
  ...
  getCallAmount() {
    ...
    return Object.keys(this.currentGame.players).reduce((a,b) => {
      const bet = this.currentGame.players[b].bet;
      if(bet > a && !this.currentGame.players[b].hasFolded) {
        a = bet;
      }
      return a;
    },0) - this.currentPlayer.bet;
  }

  call() {
    if(!this.currentPlayer || this.tempBet > this.currentPlayer.chips) return;
    
    this.tempBet = this.getCallAmount();
    this.correctTempBet();
    this.setTempBetText();
  ...
  updatePlayerSlices(players, game) {
    for(let i = 0; i < 8; i++) {
      ...
      // Skip if this is a folded player marked as winner (shouldn't happen with server fixes)
      if(game.winner && game.winner._id === id && player.hasFolded) {
        continue;
      }
      ...
      this.setText(playerSection.querySelector("._lastHand"), player.lastHand && player.lastHand.descr ? player.lastHand.descr : "");
      ...
      if(game.isStarted && !game.winner) {
        ...
        // Show hand description for all players
        this.setText(playerSection.querySelector("._lastHand"), 
          player.lastHand.descr + (player._id === game.winner._id ? ", winner!" : ""));
        
        // Show cards for:
        // 1. The winner (always)
        // 2. Current player (always)
        // 3. Other non-folded players
        const shouldShowCards = (player._id === game.winner._id) || 
                              (id === window.user.id) || 
                              (!player.hasFolded && game.winner._id !== player._id);
  ...
  syncGame(game) {
    ...
    if(this.params.debug === "true") {
      ...
      Object.keys(game.players).forEach(playerId => {
        totalChips += game.players[playerId].chips;
        totalChips += game.players[playerId].bet;
      });
  ...
  getTableHTML() {
    ...
    const playerSection = Array.from({length: 8}, (v, i) => i).map(i => `
      <a-entity class="_playerPosition${i}" rotation="0 ${45*i} 0">
    ...
    const html = `
      <a-box scale="0.1 0.1 0.1" color="red" class="resetGame" data-raycastable sq-collider sq-interactable position="0 0 0"></a-box>
