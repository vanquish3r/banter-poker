    ...
    if (game) {
      ...
      if (player && this.playerIsCzar(player, game)) {
        ...
        if (nonFoldedPlayers.length === 1) {
          console.log("nonFoldedPlayers is " + nonFoldedPlayers[0].name);
          const winnings = Object.keys(game.players).reduce((a, b) => {
            a += game.players[b].bet;
            game.players[b].bet = 0; // Zero out bets after adding to winnings to prevent chip inflation by bet totals (temporary hack fix)
            return a;
          }, 0);
