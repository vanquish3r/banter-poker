...
class GameServer {
  constructor() {
    ...
    this.setupServer();
    this.games = {};
    this.INITIAL_CHIPS = 100; // Constant for initial chip count
  }
    ...
    if(game) {
      ...
      if(player) {
        const callAmount = Object.keys(game.players).reduce((a,b) => {
          ...
          if(game.players[b].bet > a && !game.players[b].hasFolded) {
            a = bet;
          }
          return a;
        },0);
  ...
  check(ws) {
    ...
    if(game) {
      const player = game.players[ws.u.id];
      if(player && this.canCheck(game, player) && this.playerIsCzar(player, game)) {
        player.hasBet = true;
        this.nextRound(ws, game);
      }
    }
  ...
  winner(game, player) {
    if (player && !player.hasFolded) {
      game.winner = player;
      setTimeout(() => {
        this.resetGame(null, game, false); // Start new round after delay
      }, 5000);
  ...
  fold(ws) {
    ...
    if(game) {
      ...
      if(player && this.playerIsCzar(player, game)) {
        ...
        const nonFoldedPlayers = Object.keys(game.players).filter(p => !game.players[p].hasFolded);
        
        if(nonFoldedPlayers.length === 1) {
          const totalPot = Object.keys(game.players).reduce((sum, playerId) => {
            return sum + game.players[playerId].bet;
          }, 0);
  ...
  resetDeck() {
    return Object.keys(deck).slice().sort(() => Math.random() - 0.5).map(card => ({card, image: deck[card]}));
  }
  ...
  resetGame(ws, game, hardReset) {
    ...
    players.forEach(p => {
      ...
      game.players[p].lastHand = null; // Clear previous hand results
      game.players[p].hasBet = false;
    });
    game.pots = [];
    game.tableCards = [];
  ...
  nextCzar(game) {
     const players = Object.keys(game.players).filter(p => !game.players[p].hasFolded || p === game.czar);
  ...
  allHasBet(game, players) {
    return players.filter(p => game.players[p].hasBet).length === players.length;
  }

  canCheck(game, player) {
    const players = Object.keys(game.players);
    return !players.filter(p => player.bet < game.players[p].bet && !game.players[p].hasFolded).length;
  }
        ...
        players.forEach(p => {
          if(game.players[p].hasFolded) {
            ...
            game.players[p].lastHand = Hand.solve(
              game.players[p].cards.map(c => c.card)
                .concat(game.tableCards.map(c => c.card))
            );
          }
        });
        
        const activeHands = nonFoldedPlayers.map(p => game.players[p].lastHand);
        const winners = Hand.winners(activeHands);
        const totalPot = players.reduce((sum, playerId) => {
          return sum + game.players[playerId].bet;
        }, 0);
        
        if (winners.length > 0) {
          const firstWinnerId = nonFoldedPlayers[activeHands.indexOf(winners[0])];
          const winner = game.players[firstWinnerId];
          winner.chips += Math.floor(totalPot / winners.length);
          this.winner(game, winner);
          return; // Winner method will handle sync
        }
      } else {
        this.riverTurn(game);
  ...
  syncGame(game, ws, isReset) {
    const {players, waitingRoom, czar, isStarted, winner, pots, blinds, tableCards} = game;
    const playerIds = Object.keys(players);
    const _players = playerIds.map(d => {
      const {_id, trophies, cards, selected, name, position, connected, disconnectTime, chips, bet, hasBet, hasFolded, lastHand} = players[d];
      ...
      return {_id, trophies, cards, selected, name, position, connected, disconnectTime, chips, bet, hasBet, hasFolded, lastHand: _lastHand, canCheck};
    }).reduce((a,b) => {
      a[b._id] = b;
      return a;
    }, {});
    ...
    if(ws) {
      this.send(ws, "sync-game", 
                {players: _players, playerCount, waitingRoom, czar, isStarted, winner, pots, blinds, tableCards, isReset});
    } else {
      Object.keys(game.sockets).forEach(socket => {
