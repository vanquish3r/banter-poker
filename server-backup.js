...
class GameServer{
  ...
  fold(ws) {
    ...
    if(game) {
      ...
      if(player && this.playerIsCzar(player, game)) {
        ...
        const notFolded = players.filter(p => !game.players[p].hasFolded);
        ...
        if(notFolded.length === 1){
          ...
          game.players[notFolded[0]].chips += winnings;
          game.players[notFolded[0]].lastHand = {descr: "All others folded"}
          this.winner(game, game.players[notFolded[0]]);
        this.playSound(game, "fanfare%20with%20pop.ogg");
        }
        this.nextRound(ws, game);
      } 
      ...
      if(game.tableCards.length === 5) {
        players.forEach(p => {
          if(game.players[p].hasFolded) {
            ...
            game.players[p].lastHand = Hand.solve(game.players[p].cards.map(c=>c.card).concat(game.tableCards.map(c=>c.card)));
          }
        });
        const hands = nonFoldedPlayers.map(p => game.players[p].lastHand);
        ...
        winners.forEach(w => {
          const winner = game.players[players[hands.indexOf(w)]];
          winner.chips += Math.floor(winnings / winners.length);
        });
        console.log("total winners: ", winners.length);
        this.winner(game, game.players[players[hands.indexOf(winners[0])]]);
      }else{
        this.riverTurn(game);
  ...
  syncGame(game, ws, isReset) {
    ...
    const _players = playerIds.map(d => {
      ...
      return {_id, trophies, cards, selected, name, position, connected, disconnectTime, chips, bet, hasBet, hasFolded, lastHand, canCheck};
    }).reduce((a,b) => {
      a[b._id] = b;
      return a;
    }, {});
